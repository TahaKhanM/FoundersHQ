...
> Note: this is a design document describing the full intended product across all tiers. The README lists the subset that is actually implemented in this codebase.
...
Longer-term roadmap headlines (not implemented):

- **F16 Causal Analyst Agent** — multi-step LLM investigation with cited evidence per step.
